...
description: Shop VIP.com with smart deal finding, flash sale timing, brand navigation, price comparison guidance, and browser-based sale/detail checks.
metadata:
  clawdbot:
    emoji: "🛍️"
    requires:
...
When the user needs live VIP sale-page validation, follow the shared **browser-commerce-base** workflow:
- use `openclaw` for public sale, search, and detail pages
- capture sale timing, stock/size hints, and discount framing
- re-snapshot after size/color selection or timed sale refresh
- use screenshots for time-sensitive recommendations

Key browser extraction order on VIP:
- 商品标题
- 特卖价 / 对比价 / 到手价
- 品牌 / 活动档期
- 尺码库存提示
